...
class ExportController extends ControllerBase 
{
	...
	public function indexAction()
	{
        ...
        $this->statistics();
        $this->view->setVar('Type', 'export');
		$this->view->setVar('Export', Export::find(array("order" => "datetime DESC"))); 
	}

	public function insertAction()
	{
	    $Auth = $this->session->get('auth');
        if ($this->request->isPost() AND $this->request->getPost('export') AND $Auth) {
	...
	public function updateAction($id)
	{
        ...
        if ($this->request->isPost() AND $this->request->getPost('export') AND $Auth) {
            ...
            $totals = ($l500000 * 500000) + ($l200000 * 200000) + ($l100000 * 100000) + ($l50000 * 50000) + ($l20000 * 20000) + ($l10000 * 10000) + ($l5000 * 5000) + ($l2000 * 2000) + ($l1000 * 1000);
            //       
            $find = Export::findFirst($this->request->getPost('id'));
            $find->user = $Auth['login'];
            $find->export = $export;
            $find->totals = $totals;
            $find->l500000 = $l500000;
	...
	public function editAction($id)
	{
        ...
        $find = Export::findFirst($id);
        foreach ($find  as $key => $value) {
            $this->view->setVar($key, $value);
        }
        $this->forward('export/index');
    }
